(function() {
  ...
  _.keys(join_methods).forEach(function(method) {
  ...
  _.each(compounds, function(value, key) {
  ...
  _.each(compounds, function(sql_keyword, clause_id) {
  ...
  Insert.prototype.values = function values() {
    if (this._split_keys_vals_mode) {
      ...
      _.each(outer_arr, (function(args, outer_ix) {
  ...
  for (var name in binary_ops) {
    ...
    _.each(quantifiers, (function(name, quantifier) {
  ...
  sql._extension = function () {
    ...
    _.each(_.keys(sql), function(prop_name) {
